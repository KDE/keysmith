...
CommandLineOptions::CommandLineOptions(QCommandLineParser &parser, bool parseOk, QObject *parent)
    : QObject(parent)
    , m_parseOk(parseOk)
    , m_errorText(parseOk ? QString() : parser.errorText())
    , m_parser(parser)
{
}
...
bool Proxy::enable(Keysmith *keysmith)
{
    ...
    if (m_keysmith) {
        qCDebug(logger) << "Not (re)initialising proxy with new Keysmith instance:" << keysmith << "Already initialised with:" << m_keysmith;
        return false;
    }
